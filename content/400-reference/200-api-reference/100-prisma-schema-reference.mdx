...
The [MongoDB connector](/concepts/database-connectors/mongodb) does not support the `Unsupported` type.
...
- When introspecting a databases that contains unsupported types, Prisma will always warn:
...
- In [2.17.0](https://github.com/prisma/prisma/releases/tag/2.17.0) and later, Prisma automatically adds `@ignore` to fields that _refer to_ invalid models when you introspect.

#### Examples

The following example demonstrates manually adding `@ignore` to exclude the `email` field from the Prisma Client:
...
Add `@@ignore` to a model that you want to exclude from the Prisma Client (for example, a model that you do not want Prisma users to update). Ignored models are excluded from the generated Prisma Client.
...
### <inlinecode>sequence()</inlinecode>

<Admonition type="info">

**Only supported by CockroachDB** <br />
The sequence function is only supported by [CockroachDB connector](/concepts/database-connectors/cockroachdb).
...
<TabbedContent tabs={[<FileWithIcon text="CockroachDB only" icon="database"/>]}>
<tab>
...
Defines a [composite type](/concepts/components/prisma-schema/data-model#defining-composite-types) <span class="concept"></span>.
